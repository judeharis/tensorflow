...
#include "tensorflow/lite/delegates/utils/tconv_fpga_delegate/accelerator/driver/gemm_driver.h"
#include "tensorflow/lite/delegates/utils/tconv_fpga_delegate/util.h"
...
public:
  ...
  TfLiteStatus Eval(TfLiteContext *context, TfLiteNode *node) override {
    ...
    for (int i = 0; i < node_count; i++) {
      ...
      // precal_sum_load_pad(input_data, width, depth, inb0, inb1, inb2, inb3,
      //                     in_sum1, in_sum2, in_sum3, in_sum4);
      // Replace precal_sum_load_pad
      int dm = 0;
      for (int i = 0; i < w / 4; i++) {
        int id = i * d4;
        int i0 = id;
...
TCONVFPGADelegateOptions TfLiteTCONVFPGADelegateOptionsDefault() {
  ...
  // Just assign an invalid builtin code so that this tconvfpga test delegate
...
TfLiteTCONVFPGADelegateCreate(const TCONVFPGADelegateOptions *options) {
  ...
  return tflite::TfLiteDelegateFactory::CreateSimpleDelegate(
      std::move(tconvfpga), kTfLiteDelegateFlagsAllowDynamicTensors);
}
