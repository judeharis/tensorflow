...
void saveData(gemm_driver &gd, bool inputs, int inl0, int inl1, int inl2, int inl3){
    ...
    string filename = "model_l"+ std::to_string(gd.t.layer)+ "_w" + std::to_string(gd.t.layer_weight_tile);
    if(inputs) filename = "model_l"+ std::to_string(gd.t.layer)+ "_w" + std::to_string(gd.t.layer_weight_tile) + "_i" + std::to_string(gd.t.layer_input_tile);
...
void Set_Results(gemm_driver &gd) {
    ...
    dma_set<int>(gd.dma0, S2MM_LENGTH,dma_buffer_size);
    dma_set<int>(gd.dma1, S2MM_LENGTH,dma_buffer_size);
    dma_set<int>(gd.dma2, S2MM_LENGTH,dma_buffer_size);
    dma_set<int>(gd.dma3, S2MM_LENGTH,dma_buffer_size);
}
